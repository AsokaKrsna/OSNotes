...
import com.osnotes.app.ui.tools.ShapeAnnotation
...
class AnnotationManager @Inject constructor(
    ...
    private fun getPointsForShape(shape: ShapeAnnotation): List<Point> {
         return when (shape.type) {
            ...
            ShapeType.TRIANGLE -> {
                // Equilateral-ish triangle within bounding box
                ...
                listOf(
                    Point(topX, topY),           // Top vertex
                    Point(rightX, bottomY),      // Bottom right
                    Point(leftX, bottomY),       // Bottom left
                    Point(topX, topY)            // Close path
                )
            }
        }
    }
